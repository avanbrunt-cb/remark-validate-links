...
/**
 * @typedef BlogConfig
 *   Configuration for blog post URL handling.
 * @property {boolean | null | undefined} [enabled]
 *   Enable blog-specific features (default: false).
 * @property {string | null | undefined} [directory]
 *   Blog content directory relative to content directory (default: 'blog').
 * @property {boolean | null | undefined} [autoGenerateAliases]
 *   Auto-generate URL aliases from blog post titles (default: false).
 * @property {boolean | null | undefined} [dateInUrl]
 *   Whether blog URLs include date prefixes like /YYYY/MM/ (default: false).
 * @property {boolean | null | undefined} [compressMultipleDashes]
 *   Compress multiple consecutive dashes in slugs (default: false).
 */

/**
 * @typedef HugoConfig
 *   Configuration for Hugo static site generator.
 * @property {boolean | null | undefined} [enabled]
 *   Enable Hugo-specific features (default: false).
 * @property {boolean | null | undefined} [parseFrontmatterAliases]
 *   Parse and validate Hugo aliases from YAML front matter (default: false).
 * @property {string | null | undefined} [contentDirectory]
 *   Path to Hugo content directory relative to root (default: 'content').
 * @property {Readonly<BlogConfig> | null | undefined} [blogConfig]
 *   Configuration for blog post handling.
 * @property {ReadonlyArray<string> | null | undefined} [resourcePaths]
 ...
 * @property {Readonly<HugoConfig> | null | undefined} [hugoConfig]
 *   Configuration for Hugo static site generator support.
 * @property {ReadonlyArray<string> | null | undefined} [extraLandmarks]
 *   Additional landmarks that should be considered valid (e.g., ['/blog', '/showcase']).
 * @property {boolean | null | undefined} [stripMarkdownExtensions]
 *   Whether to strip .md extensions for extension-agnostic links (default: false).
 * @property {boolean | null | undefined} [normalizeTrailingSlash]
 *   Whether to remove trailing slashes from paths (default: false).
 */
 ...
 *   URL configuration.
 * @property {Readonly<HugoConfig>} hugoConfig
 *   Hugo configuration.
 * @property {boolean} stripMarkdownExtensions
 *   Whether to strip .md extensions.
 * @property {boolean} normalizeTrailingSlash
 *   Whether to normalize trailing slashes.
 */
...
export default function remarkValidateLinks(options, fileSet) {
  ...
  return async function (tree, file) {
    ...
    // Initialize Hugo configuration
    const hugoConfig = settings.hugoConfig || {}
    const stripMarkdownExtensions = settings.stripMarkdownExtensions || false
    const normalizeTrailingSlash = settings.normalizeTrailingSlash || false
    const extraLandmarks = settings.extraLandmarks || []
    ...
    // Add extra landmarks from configuration
    if (extraLandmarks.length > 0 && root) {
      for (const landmark of extraLandmarks) {
        addLandmarks(path.resolve(root, '.' + landmark), '')
      }
    }
    ...
    visit(tree, function (node) {
      // Process Hugo front matter (YAML nodes)
      if (
        hugoConfig.enabled &&
        node.type === 'yaml' &&
        'value' in node &&
        ...
        // Parse explicit aliases from front matter
        if (hugoConfig.parseFrontmatterAliases) {
          try {
            const parsedAliases = parseAliases(node.value)
            if (parsedAliases) {
        ...
        // Auto-generate blog aliases from title
        const blogConfig = hugoConfig.blogConfig || {}
        if (
          blogConfig.enabled &&
          blogConfig.autoGenerateAliases &&
          absolute.includes(
            path.join(
              hugoConfig.contentDirectory || 'content',
              blogConfig.directory || 'blog'
            )
          )
        ) {
          const title = extractTitle(node.value)
          if (title) {
            let slug = slugger.slug(title)

            if (blogConfig.compressMultipleDashes) {
              slug = compressDashes(slug)
            }
    ...
    // Copy landmarks for aliases
    if (aliases && aliases.length > 0) {
      for (const alias of aliases) {
        const info = urlToPath(alias, state, 'link')

        if (info) {
          let aliasPath = info.filePath
          let sourcePath = absolute

          // Strip .md extension if configured
          if (stripMarkdownExtensions) {
            if (path.extname(aliasPath) === '.md') {
              aliasPath = aliasPath.slice(0, -3)
            }
          ...
          const sourceMarks = landmarks.get(sourcePath)
          if (sourceMarks) {
            landmarks.set(aliasPath, new Map(sourceMarks))
          }
        }
      }
    }
    ...
    function addLandmark(filePath, hash) {
      // Strip .md extension if configured
      if (stripMarkdownExtensions && path.extname(filePath) === '.md') {
...
async function cliCompleter(set) {
  // Clear cache for new batch to ensure fresh results
  await checkAll(set.valueOf(), true)
}
 ...
 *   Whether to clear file existence cache for new batch.
 * @returns {Promise<undefined>}
 *   Nothing.
 */
async function checkAll(files, clearCache) {
  // Merge landmarks.
  /** @type {Landmarks} */
  const landmarks = new Map()
  ...
  // Pass clearCache flag to clear cache at start of new batch.
  await checkFiles(landmarks, [...references.keys()], clearCache)
...
function urlToPath(value, state, type) {
  ...
  if (value.charAt(0) === slash) {
    // Hugo-specific path resolution
    if (state.hugoConfig.enabled && state.root) {
      const blogConfig = state.hugoConfig.blogConfig || {}
      const resourcePaths = state.hugoConfig.resourcePaths || []
      const contentDir = state.hugoConfig.contentDirectory || 'content'

      // Check for resource paths (static resources)
      for (const resourcePath of resourcePaths) {
        if (value.startsWith(resourcePath)) {
          return normalize(value, state)
        }
      }

      // Check for blog date pattern (/YYYY/MM/post-name)
      if (blogConfig.enabled && blogConfig.dateInUrl && isBlogDatePath(value)) {
        const blogDir = blogConfig.directory || 'blog'
        const resolved = path.resolve(
          state.root,
          resolveHugoPath(value, contentDir, blogDir)
        )
        return normalize(resolved, state)
      }

      // General Hugo content path (/docs/guide -> content/docs/guide)
      const resolved = path.resolve(state.root, contentDir + value)
      return normalize(resolved, state)
    }
...
function normalize(url, state) {
  ...
  // Strip trailing slash if configured
  if (state.normalizeTrailingSlash && filePath.endsWith(slash)) {
