...
export async function findReferences(ctx) {
  ...
  // Add non file landmarks
  addLandmark('/blog', '')
  addLandmark('/showcase', '')
  addLandmark('/resources/query_overview.pdf', '')
  addLandmark('/resources/5.2-process-document.png', '')
  addLandmark('/resources/6.1-process-document.png', '')
  ...
  visit(ctx.tree, (node) => {
    ...
    if (typeof(node.value) === 'string' && node.value.includes('aliases:')){
      try {
        aliases = JSON.parse(node.value.substring(node.value.indexOf("["), node.value.lastIndexOf("]") + 1))
      } catch {
        throw "aliases is not a valid JSON array"
      }
    }

    // Create alias for blog
    if (typeof(node.value) === 'string' && node.value.includes('title:') && absolute.includes('content/blog')){
      // Extract title for alias name
      // title: "Example title"
      const start = node.value.indexOf('"')
      const end = node.value.indexOf('"', start + 1)
      aliases = [slugs.slug(node.value.substring(start + 1, end)).replace(/-+/g, '-')]
    }
  ...
  // Copy landmarks from absolute file for aliases
  if (aliases) {
    for (const alias of aliases) {
      const info = urlToPath(alias, config, 'link')
      let aliasRef = path.extname(info.filePath) === '.md' ? info.filePath.slice(0, -3) : info.filePath
      let absoluteRef = path.extname(absolute) === '.md' ? absolute.slice(0, -3) : absolute

      // Strip .md for cleaner references
      landmarks[aliasRef] = landmarks[absoluteRef]
    }
  }
  ...
  function addLandmark(filePath, hash) {
    // Slice .md for better link reference
  ...
  function addReference(filePath, hash, node) {
    // Slice extension for better link reference
    if(path.extname(filePath) === '.md') {
      filePath = filePath.slice(0, -3)
    }
...
function urlToPath(value, config, type) {
  ...
  if (value.charAt(0) === slash) {
    ...
    // Check if file is a blog reference
    if (!isNaN(value.substring(1, 5)) && !isNaN(value.substring(6, 8))) {
      return normalize(config.root + 'content/blog' + value.substring(8), config)
    }

    // Check if file is a resource reference
    if (value.includes('/resources/')) {
      return normalize(value, config)
    }

    return normalize(config.root + 'content' + value, config)
  }
